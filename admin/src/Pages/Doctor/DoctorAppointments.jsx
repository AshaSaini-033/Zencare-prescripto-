...
const DoctorAppointments = () => {
    ...
    const {calculateAge,currancySymbol}=useContext(AppContext)
    console.log(dToken)
    useEffect(()=>{
        if(dToken){
            getAppointments()
      ...
      <div className='bg-white border rounded text-sm max-h-[80vh] min-h-[50vh] overflow-y-scroll'>
        ...
        </div>
       {
        appointments.reverse().map((item,index)=>(
            <div key={index} className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 py-3 px-6 border-b text-gray-600 items-center hover:bg-gray-100'>
                ...
                <div>
                    ...
                    <p>{currancySymbol} {item.amount}</p>
                  <div>
                    {
                        item.cancelled ?
